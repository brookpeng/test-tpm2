...
def rsa_sign_with_key(rsa, data):
	...
	ret = _libcrypto.EVP_DigestInit(context, _libcrypto.EVP_sha1())
	if ret != 1:
		print('error EVP_DigestInit()')
	ret = _libcrypto.EVP_DigestUpdate(context, data_void_ptr, len(data))
	if ret != 1:
...
def sign_hash(digest):
	...
	output = rsa_sign_with_key(rsa, digest)
